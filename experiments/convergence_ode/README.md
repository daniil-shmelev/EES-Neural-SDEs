...
Run from the repository root in a Python 3.12+ environment with Diffrax, georax, diffrax-lowstorage, JAX, NumPy, SciPy, and Matplotlib:
...
The two-by-two figure uses the local `plotting.plot_error_curve` helper and the shared STIX settings from `experiments.plotting`. Rows correspond to CF-EES(2,5) and CF-EES(2,7); columns show forward and recovery errors. Crimson crosses are measurements; blue lines have prescribed slopes. All measurements remain plotted, including those near roundoff.
