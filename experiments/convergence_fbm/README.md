...
- **SO(3) RDE** (`scripts/so3_*.py`, `scripts/reversibility_*.py`, `scripts/extra_geometries.py`) — same Redmann–Riedel test bed, lifted to a Lie-group RDE driven by 2-dimensional fBm. Verifies the predicted $(p+1)\alpha - 1$ forward and antisymmetric-order rates of $\mathrm{CF\text{-}EES}(2,5)$.
...
python -m experiments.convergence_fbm.scripts.reversibility_sweep
```
...
| `results/reversibility_sweep_results.{json,csv}` | data backing the reversibility sweep |
