...
if __name__ == "__main__":
    ...
    if os.path.isfile('plots/gbm_reversible_heun/grad_err.pickle'):
        with open('plots/gbm_reversible_heun/grad_err.pickle', 'rb') as handle:
    ...
    if os.path.isfile('plots/gbm_mcf_euler/grad_err.pickle'):
        with open('plots/gbm_mcf_euler/grad_err.pickle', 'rb') as handle:
            mcf_euler = pickle.load(handle)
        print(np.mean(mcf_euler))
        plt.plot(mcf_euler, label = "MCF Euler", color="magenta", linestyle = "--", zorder = 3)

    if os.path.isfile('plots/gbm_mcf_midpoint/grad_err.pickle'):
        with open('plots/gbm_mcf_midpoint/grad_err.pickle', 'rb') as handle:
    ...
    if os.path.isfile('plots/gbm_ees25/grad_err.pickle'):
        with open('plots/gbm_ees25/grad_err.pickle', 'rb') as handle:
