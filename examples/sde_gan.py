...
class Generator(torch.nn.Module):
    def __init__(self, data_size, initial_noise_size, noise_size, hidden_size, mlp_size, num_layers, method, dt):
        super().__init__()
    ...
    def forward(self, ts, batch_size):
        ...
        ###################
        xs = torchsde.sdeint_adjoint(self._func, x0, ts, method=self.method, dt=self.dt,
...
class Discriminator(torch.nn.Module):
    def __init__(self, data_size, hidden_size, mlp_size, num_layers, method, dt):
        super().__init__()
    ...
    def forward(self, ys_coeffs):
        ...
        h0 = self._initial(Y0)
        hs = torchcde.cdeint(Y, self._func, h0, Y.interval, method=self.method, backend='torchsde', dt=self.dt,
                             adjoint_method='adjoint_' + self.method,
...
def get_data(batch_size, device):
    ...
    class CIRSDE(torch.nn.Module):
        ...
        def g(self, t, y):
            # Diffusion: sigma * sqrt(y)
            # Ensure numerical stability by clamping y >= 0
...
###################
def plot(ts, generator, dataloader, num_plot_samples, plot_locs, DIR):
    # Get samples
    ...
    for i,prop in enumerate(plot_locs):
        ...
        plt.tight_layout()
        plt.savefig(DIR + "/sde_gan_"+ str(i) + ".pdf")
        plt.clf()
        #plt.show()

        with open(DIR + "/sde_gan_"+ str(i) + ".pickle", 'wb') as handle:
            pickle.dump((real_samples_time.cpu().numpy(), generated_samples_time.cpu().numpy()), handle)
    ...
    for real_sample_ in real_samples:
        ...
        real_first = False
        SAVE_1.append((ts.cpu(), real_sample_.cpu()))
    for generated_sample_ in generated_samples:
        ...
        SAVE_2.append((ts.cpu(), generated_sample_.cpu()))
    plt.legend()
    plt.title(f"{num_plot_samples} samples from both real and generated distributions.")
    plt.tight_layout()
    plt.savefig(DIR + "/sde_gan.pdf")
    plt.clf()
    #plt.show()

    with open(DIR + "/sde_gan.pickle", 'wb') as handle:
        pickle.dump((SAVE_1, SAVE_2), handle)
...
def main(
        ...
        hidden_size=8,        # How big the hidden size of the generator SDE and the discriminator CDE are.
        mlp_size=8,           # How big the layers in the various MLPs are.
    ...
    # Models
    generator = Generator(data_size, initial_noise_size, noise_size, hidden_size, mlp_size, num_layers, method, dt).to(device)
    discriminator = Discriminator(data_size, hidden_size, mlp_size, num_layers, method, dt).to(device)
    ...
    for step in trange:
        ...
        if (step % steps_per_print) == 0 or step == steps - 1:
            ...
            if step > swa_step_start:
                ...
                trange.write(f"Step: {step:3} Loss (unaveraged): {total_unaveraged_loss:.4f}")
            LOSS.append(total_unaveraged_loss)
    generator.load_state_dict(averaged_generator.module.state_dict())
    discriminator.load_state_dict(averaged_discriminator.module.state_dict())

    _, _, test_dataloader = get_data(batch_size=batch_size, device=device)

    with open(DIR + '/sde_gan_loss.pickle', 'wb') as handle:
        pickle.dump(LOSS, handle)

    plot(ts, generator, test_dataloader, num_plot_samples, plot_locs, DIR)
...
if __name__ == '__main__':
    ...
    DIR = "plots/sde_gan_" + args.method

    with open(DIR + "/time.txt", "w") as text_file:
        text_file.write("Total time: " + str(end - start))
