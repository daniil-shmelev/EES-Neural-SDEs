...
if __name__ == "__main__":
    ...
    if os.path.isfile('plots/gbm_mcf_euler/mse.pickle'):
        with open('plots/gbm_mcf_euler/mse.pickle', 'rb') as handle:
            mcf_euler = pickle.load(handle)
        print(mcf_euler[-1])
        plt.plot(mcf_euler, label = "MCF Euler", color="magenta", linestyle = "--", zorder = 3)

    if os.path.isfile('plots/gbm_mcf_midpoint/mse.pickle'):
        with open('plots/gbm_mcf_midpoint/mse.pickle', 'rb') as handle:
